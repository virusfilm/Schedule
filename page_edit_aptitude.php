...
<?
	...
	while($row = mysql_fetch_array($query))
	{
			...
			$str="<tr hieght=500><td>รหัสอาจารย์ :</td><td>".$row['T_id']."</td></tr>";
			...
			$str4="<tr><td>รุ่นธุดงค์ :</td><td>".$row['T_Thudong']."</td></tr>";
			$str5="<tr><td>รุ่นอาจาริยสา :</td><td>".$row['T_Ajariyasa']."</td></tr>";
			$str6="<tr><td rowspan=".$c ." valign=top>หัวข้อที่ถนัด :</td></tr>";

			for($k=0;$k<$j;$k++)
			{
			  if($Tch_id[$k] == $row['T_id'])
			  {
			  	for($p=0;$p<$l;$p++)
				{
					if($Tpc_id[$k] == $Tpc_tid[$p])
			  		{
			  		$str7=$str7."<tr><td>".$TpcL[$p].".".$TpcM[$p]." ".$Tpc_n[$p]."</td></tr>";
					$topic_id[] = $Tpc_id[$k];
					}
				}
			  }
			}
			if($row['T_Type'] =='TM'){$type_t="ครูหลัก";}if($row['T_Type'] =='TC'){$type_t="ครูฝึกสอน";}if($row['T_Type'] =='TS'){$type_t="ครูฝึกสรุป";}
...
?>
    	<tr  height="30"  bgcolor="#d8ffca">
            <td align="center">
			<?
			 $sql_ap="select * from aptitude where T_id='$row[T_id]' ";
			 ...
			 $num = mysql_num_rows($query_ap);
				...
				if($num ==0)
				{
				echo "<a href\"#\" onMouseover=\"showhint('$alert', this, event, '150px','100px')\"><font color=#FF0000>".$row['T_id']."</font></a>";
				}
				else
				{
				echo $row['T_id'];
			...
			<td><? 
				if($row['T_Type'] =='TM')
				{
					if($num ==0)
					{
						echo "<a href\"#\" onMouseover=\"showhint('$alert', this, event, '150px','100px')\"><font color=#FF0000>ครูหลัก</font></a>";
					}
					else
					{
						echo"ครูหลัก";
				...
				if($row['T_Type'] =='TC')
				{
					if($num ==0)
					{
						echo "<a href\"#\" onMouseover=\"showhint('$alert', this, event, '150px','100px')\"><font color=#FF0000>ครูฝึกสอน</font></a>";
					}
					else
					{
						echo"ครูฝึกสอน";
				...
				if($row['T_Type'] =='TS')
				{
					if($num ==0)
					{
						echo "<a href\"#\" onMouseover=\"showhint('$alert', this, event, '150px','100px')\"><font color=#FF0000>ครูฝึกสรุป</font></a>";
					}
					else
					{
						echo"ครูฝึกสรุป";
            ...
            <td align="center"><a href="index.php?frm=frm_edit_aptitude&id=<?=$row['ID']; ?> " onMouseover="showhint('<? echo $str_cmp;?>', this, event, '150px')"><img src="images/edit.png" alt="" title="" border="0" /></a></td>
        </tr>
  ...
  printf("มีอาจารย์ %d ท่าน / ",$total);
			...
			for($i=1;$i<=$page;$i++)
			{
				...
				{ //ถ้าตัวแปล page ตรง กับ เลขที่วนได้
					echo "<font color='#ff0000'>[<a href='?frm=edit_aptitude&start=".$limit*($i-1)."&page=$i' style='text-decoration:none;'><B>$i</B></a>]</font>"; //ลิ้งค์ แบ่งหน้า เงื่อนไขที่ 1
				}
				else
				{
					echo "[<a href='?frm=edit_aptitude&start=".$limit*($i-1)."&page=$i' style='text-decoration:none;'>$i</a>]"; //ลิ้งค์ แบ่งหน้า เงื่อนไขที่ 2
				}
			}
?>    
</td>
