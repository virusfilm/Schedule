...
?>
    	<tr height="30"  bgcolor="#d8ffca">
            ...
            <td align="center"><a href="index.php?frm=delete_teacher&id=<? echo $row['ID']; ?>&pic=<? echo $row['T_Pic']; ?>&T_id=<? echo $row['ID']; ?>&type_t=<?=$row['T_Type'] ?>" onMouseover="showhint('<? echo $str_cmp;?>', this, event, '150px')" onClick="return confirm('คุณต้องการลบข้อมูลหรือไม่ ?');"><img src="images/trash.png" alt="" title="" border="0" /></a></td>
        </tr>
  ...
  printf("มีอาจารย์ %d ท่าน / ",$total);
			...
			for($i=1;$i<=$page;$i++)
			{
				...
				{ //ถ้าตัวแปล page ตรง กับ เลขที่วนได้
					echo "<font color='#ff0000'>[<a href='?frm=delete_teacher&start=".$limit*($i-1)."&page=$i' style='text-decoration:none;'><B>$i</B></a>]</font>"; //ลิ้งค์ แบ่งหน้า เงื่อนไขที่ 1
				}
				else
				{
					echo "[<a href='?frm=delete_teacher&start=".$limit*($i-1)."&page=$i' style='text-decoration:none;'>$i</a>]"; //ลิ้งค์ แบ่งหน้า เงื่อนไขที่ 2
				}
			}
?>    
</td>
