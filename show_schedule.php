...
<th  align="right"><a href="download_schedule.php?mode=<?=$mode ?>" target="_blank"><img src="images/download_wodrd.gif" width="20" height="20" title="ดาวน์โหลด"/></a></th>
</tr>
</table><br>
<? }?>
...
for($y=1;$y<=count($dat);$y=$y+$S){//$D=0;
		...
		if($day=="Sat"){
			$E=0;
					...
					if($num_ncnfrm != 0) { 		 ?>
						<td align="center"><a href="#"  onClick="window.open('frm_add_schedule.php?id_topic=<?=$id_topic[$t]; ?>&id_date=<?=$dat[$y]; ?>&genaration=<?=$mode; ?>&sum=<?=$S; ?>', 'joe',config='height=500,width=700,menubar=0,resizable=0')"><img src="images/edit.png" alt="" title="" border="0" /></a></td>
						<?	}
							break;
...
<br>
<? if($num_ncnfrm != 0) { ?>
<center><a href="?mode=<?=$mode ?>&confirm=1" onClick="return confirm('คุณต้องการยืนยันตารางสอนหรือไม่ ?');">ยืนยัน</a></center>
<? } ?>
<? if($num_cnfrm != 0) { ?>
<center><a href="?mode=<?=$mode ?>&confirm=2" onClick="return confirm('คุณต้องการแก้ไขตารางสอนหรือไม่ ?');">แก้ไขตารางสอน</a></center>
<? } ?>
<?
